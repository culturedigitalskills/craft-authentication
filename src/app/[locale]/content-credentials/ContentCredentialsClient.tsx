...
import {
    ShieldCheck, ShieldAlert, Key, RefreshCw, AlertTriangle,
    FileUp, FileCheck, Activity, ChevronRight, CheckCircle2,
    XCircle, Clock, Info, ExternalLink, HelpCircle
} from 'lucide-react'
...
export function ContentCredentialsClient({ userId }: ContentCredentialsClientProps) {
    ...
    const fetchStatus = useCallback(async () => {
        try {
            ...
            const data: C2PAStatus = await res.json()
            setStatus(data)
        } catch (err: any) {
    ...
    const handleConfigureC2PA = async () => {
        ...
        setError(null)
        try {
            const res = await fetch('/api/c2pa/setup', { method: 'POST' })
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
                ...
                <p className="text-muted-foreground animate-pulse">Loading Content Credentials status...</p>
            </div>
        )
    }
        ...
        <div className="space-y-12">
            ...
            <div className="max-w-3xl space-y-4">
                ...
                <p className="text-lg text-muted-foreground">
                    Protect and prove your authorship. By signing your crafts with standard-compliant cryptographically secure Content Credentials (C2PA), anybody can verify the authenticity, history, and creator details of your works.
                </p>
            </div>
                    ...
                    {!status?.vaultInitialized && (
                        <div className="rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 space-y-6 shadow-sm">
                            <div className="flex items-center justify-between border-b border-border/60 pb-4">
                                ...
                                <span className="text-xs bg-muted px-2 py-0.5 rounded text-muted-foreground font-semibold">Inactive</span>
                            </div>
                            <p className="text-sm text-muted-foreground leading-relaxed">
                                Content Credentials require your secure vault to be initialized first. The vault manages and encrypts your private cryptographic signing keys server-side using KMS.
                            </p>
                            <Button 
                                className="w-full"
                    ...
                    {status?.vaultInitialized && !status.c2paInitialized && (
                        <div className="rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 space-y-6 shadow-sm">
                            <div className="flex items-center justify-between border-b border-border/60 pb-4">
                                ...
                                <span className="text-xs bg-amber-100 text-amber-800 px-2 py-0.5 rounded font-semibold">Setup Pending</span>
                            </div>
                            <p className="text-sm text-muted-foreground leading-relaxed">
                                Your secure vault is initialized! Now, activate Content Credentials to generate your personal X.509 certificate and cryptographic keys. Once active, your uploads will be automatically signed.
                            </p>
                            <Button 
                                className="w-full"
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 space-y-6 shadow-sm relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between border-b border-border/60 pb-4">
                                    ...
                                    <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded font-semibold">Active</span>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-start justify-between gap-4 border-t border-border/60 pt-4">
                                        <div className="space-y-0.5">
                                            <label htmlFor="auto-renew" className="text-sm font-semibold text-foreground cursor-pointer">
                                                Auto-renew Certificate
                                            </label>
                                            <p className="text-xs text-muted-foreground">
                                                Automatically rotate and sign keys before certificate expiration.
                                            </p>
                                        </div>
                                        <input
                                            id="auto-renew"
                            ...
                            {status.needsRenewal && (
                                <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-6 space-y-4 shadow-sm animate-pulse">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-amber-900 text-sm">Certificate Expiring Soon</h4>
                                            <p className="text-xs text-amber-800 mt-1 leading-relaxed">
                                                Your signing credentials certificate is in its 30-day expiration window. Please renew it now to ensure uninterrupted validation of your uploads.
                                            </p>
                                        </div>
                                    </div>
                                    <Button 
                    ...
                    <div className="rounded-2xl border border-border bg-card/60 backdrop-blur-sm p-6 shadow-sm space-y-6">
                        <div className="border-b border-border/60 pb-4">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                Drop any signed image file to inspect its cryptographic claims and provenance.
                            </p>
                        </div>
                        ...
                        >
                            {verifying ? (
                                <div className="flex flex-col items-center py-6">
                                    ...
                                    <p className="text-sm text-foreground font-medium">Reading C2PA Manifest...</p>
                                    <p className="text-xs text-muted-foreground mt-1">Running WebAssembly validator</p>
                                </div>
                            ) : (
                            ...
                            <div className="space-y-6 border-t border-border/60 pt-6">
                                ...
                                {verifyResult.hasManifest ? (
                                    verifyResult.authentic ? (
                                        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 flex items-start gap-3">
                                            ...
                                            <div className="space-y-1">
                                                <h4 className="font-bold text-sm">Authentic Credentials Secured</h4>
                                                <p className="text-xs text-emerald-800 leading-relaxed">
                                                    Signature matches the asset contents perfectly. The certificate chain is validated against your registry.
                                                </p>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="p-4 rounded-2xl bg-destructive/5 border border-destructive/20 text-destructive flex items-start gap-3">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-xs text-muted-foreground leading-relaxed">
                                                    Content credentials exist, but signature verification or certificate checks failed.
                                                </p>
                                    ...
                                    <div className="p-4 rounded-2xl bg-muted border border-border text-muted-foreground flex items-start gap-3">
                                        ...
                                        <div className="space-y-1">
                                            <h4 className="font-bold text-sm text-foreground">No Credentials Found</h4>
                                            <p className="text-xs leading-relaxed">
                                                This image is unsigned and does not contain a C2PA manifest. It has no verifiable history or origin.
                                            </p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {/* Timeline of Edits */}
                                {verifyResult.hasManifest && verifyResult.assertions && verifyResult.assertions.length > 0 && (
                                            ...
                                            {verifyResult.assertions.map((assertion, idx) => (
                                                <div key={idx} className="relative">
                                                    ...
                                                    <div className="space-y-1">
                                                        ...
                                                        <p className="text-xs text-muted-foreground leading-relaxed">
                                                            {assertion.description || `Operation performed using ${assertion.softwareAgent}.`}
                                                        </p>
                                                    </div>
                                                </div>
                                            ))}
